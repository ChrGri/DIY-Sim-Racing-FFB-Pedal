...
// target cycle time for pedal update task, to get constant cycle times,
// required for FIR filtering
...
#define PUT_TARGET_CYCLE_TIME_IN_US_U32                                        \
  (DAP_MICROSECONDS_PER_SECOND_U32 / 2000U)
...
#define MAXIMUM_STEPPER_SPEED_U32                                              \
  (uint32_t)MAX_SPEED_IN_HZ // 100000//  max steps per second, see
...
// MCP4725 pins removed on PCBA V2 as GPIO 4 is dedicated to
// BRAKE_RESISTOR_PIN_U8
...
#define FFB_ADS1220_CLK                                                        \
  7 // Takt-Signal laut Schaltplan (falls in Software genutzt)
...
#define CORE_ID_JOYSTICK_TASK_U8                                               \
  (uint8_t)0 // TinyUSB runs on Core0, keep it here, otherwise race condition
...
#else
#define REPETITION_INTERVAL_PEDAL_UPDATE_TASK_IN_US_I64                        \
  (int64_t)250 // multiple of loadcell reading task to reduce clock-jitter
#endif
...
#define REPETITION_INTERVAL_ESPNOW_TASK_IN_US_I64 (int64_t)2000
