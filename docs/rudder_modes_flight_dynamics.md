...
| Parameter | Airplane Mode (Mode 0) | Airplane with Toe Brake Mode (Mode 2) | Light Helicopter (Mode 1) | Turbine Helicopter (Force Trim) |

|---|---|---|---|---|

| **Dual-Pedal Linkage** | Rigid Push-Pull ($x_R = -x_L$) | Push-Pull in Yaw; **Decoupled in Braking** | Rigid Push-Pull ($x_R = -x_L$) | Rigid Push-Pull ($x_R = -x_L$) |

| **Centering Behavior** | Aerodynamic self-centering ($q \propto V^2$) | Aerodynamic self-centering ($q \propto V^2$) | **No centering** (stays where placed) | Centered around **dynamic magnetic brake anchor** |

| **Airspeed ($V_{\text{IAS}}$) Influence** | High (floppy at $0\text{ kts}$, stiff at $V_{\text{ne}}$) | High (aerodynamic Q-feel in yaw) | Negligible on pedal stiffness | Negligible on pedal stiffness |

| **Breakout Force** | Low to moderate ($10\text{--}25\text{ N}$) | Low to moderate ($10\text{--}25\text{ N}$) | Near zero ($0\text{--}5\text{ N}$) | Moderate around trim point ($15\text{--}30\text{ N}$) |

| **Trim Mechanism** | Aerodynamic tab (shifts aero neutral) | Aerodynamic tab (shifts aero neutral) | Hand friction nut (holds pedal) | **FTR Button** (instant snap-anchor) + Beep Trim |

| **Toe Brakes** | None (pure differential yaw) | **Yes: Dual-Mode Differential Brakes** | **None** | **None** (or parking brake on wheeled helis) |

| **Engine-Out Behavior** | Large asymmetric yaw toward dead engine | Large asymmetric yaw; differential brake steering | Total loss of torque (stomp opposite pedal) | Total loss of torque (stomp opposite pedal) |
...
### 5.6 Mode 3: Continuous Hybrid Push-Pull Rudder Yaw & Differential Toe Braking (Mode 2)

In **Toe Brake** mode (`rudderMode = 2`), yaw coupling and independent differential wheel braking operate simultaneously in a seamless, state-free physical model:

```mermaid
graph TD
    A["<b>Toe Brake Mode (rudderMode = 2)</b>"] --> B["<b>Continuous Hybrid Coupling Engine</b>"]
    B --> C["<b>Push-Pull Rudder Yaw</b><br/>Active when one pedal is pushed & opposite relaxed<br/>x_remote targets (1.0 - x_local)<br/>Output: Differential Rudder Yaw (X-Axis)"]
    B --> D["<b>Compliant Differential Braking</b><br/>Active when pilot presses pedal forward (F_pilot > 2 N, x >= center)<br/>Push-pull opposition released locally<br/>Output: Independent Wheel Brakes (Y & Z Axes)"]
```

#### Mathematical Formulation:
1. **Common-Mode Lock Removal**:
   The artificial $1200\text{ N}$ barrier blocking dual forward extension is removed:
   $$F_{\text{common}} = 0\text{ N}$$

2. **Compliant Push-Pull & Braking Admittance**:
   Each pedal locally monitors pilot applied force $F_{\text{pilot}}$ and position relative to neutral center:
   $$\text{isBraking} = (F_{\text{pilot}} > 2.0\text{ N}) \;\land\; (x \ge x_{\text{center}} - 0.02)$$
   - **When active forward braking (`isBraking == true`)**:
     Push-pull opposing force and backward sync tracking pull are cleared:
     $$F_{\text{opposing}} = 0\text{ N}, \quad F_{\text{syncTracking}} = \max(0, F_{\text{syncTracking}})$$
     The pedal travels smoothly forward into the braking zone against the aerodynamic centering spring.
   - **When passive / yawing (`isBraking == false`)**:
     The pedal participates in anti-symmetric push-pull yaw:
     $$F_{\text{opposing}} = -\max(0, F_{\text{syncRemote}} - F_{\text{pilotLocal}})$$
     Advancing one pedal smoothly retracts the opposite relaxed pedal backwards ($x_{\text{target}} = 1.0 - x_{\text{remote}}$).

3. **Simultaneous 3-Axis Joystick Telemetry**:
   The Bridge outputs both differential yaw and independent wheel brakes concurrently:
   $$\text{Yaw} = \text{JOYSTICK\_CENTER} + \frac{x_R - x_L}{2}$$
   $$\text{Brake}_{\text{Left}} = x_L, \quad \text{Brake}_{\text{Right}} = x_R$$
...
The SimHub Flight Rudder interface provides dynamic, mode-adaptive control over the dual-pedal bilateral admittance system. The UI automatically reconfigures its force-deflection curve, parameter sliders, and visual telemetry depending on whether **Airplane**, **Helicopter**, or **Toe Brake** mode is selected.
...
| **Rudder Mode Selector** | Dropdown | `Airplane` / `Helicopter` / `Airplane with Toe Brake` | `rudderMode = 0` | Activates the aerodynamic self-centering admittance strategy with linear/progressive spring return and speed-dependent force scaling. |
...
1. **Full Aerodynamic Centering Curve**: Displays the complete bipolar centering spring curve with profile selection (`Linear`, `Progressive`, `S-Curve`), trim offsets, and airspeed $Q$-feel scaling.

2. **Dual Live Deflection Visualizer**: The live deflection line in SimHub dynamically reflects flight inputs across 100% of the canvas:

   - When yawing or operating single pedal inputs, the primary marker tracks differential yaw ($0.5 + 0.5 \cdot (x_R - x_L)$).

   - When both pedals are pressed forward past neutral ($x_L > 52\% \land x_R > 52\%$), dual markers (yellow for right wheel, cyan for left wheel) display independent wheel braking deflection.

3. **Simultaneous 3-Axis Joystick Reporting**:

   - **X-Axis (Rudder Yaw)**: Differential steering based on relative pedal deflection ($JOYSTICK\_CENTER + (x_R - x_L)/2$).

   - **Y-Axis (Left Wheel Brake)**: Left pedal position from $0\%$ to $100\%$.

   - **Z-Axis (Right Wheel Brake)**: Right pedal position from $0\%$ to $100\%$.

4. **Seamless Operation**: No toggle buttons or state switches needed. The pilot can taxi, apply differential toe braking to turn on the tarmac, hold both brakes at the runway hold-short line, takeoff, and perform coordinated turns in flight continuously.



#### Toe Brake Mode Parameter Breakdown:
...
| **Rudder Mode Selector** | Dropdown | `Airplane` / `Helicopter` / `Airplane with Toe Brake` | `rudderMode = 2` | Configures the pedal system to execute hybrid push-pull yaw coupling with compliant differential toe braking. |

| **Centering Profile** | Button Switcher | `Linear` / `Progressive` / `S-Curve` | Spline interpolation in `ForceCurve.cpp` | Configures aerodynamic spring gradient for yaw flight steering and braking feel. |

| **Airspeed Q-Feel Gain** | Horizontal Slider | $0\text{--}100\text{ \%}$ | Dynamic scaling factor in SimHub | Dynamically scales yaw resistance with simulator airspeed ($V_{\text{IAS}}$). |

| **Rudder Trim Offset** | Horizontal Slider | $-25\text{\%}\text{ to }+25\text{\%}$ | `rudderOffsets_st.trimOffset_01` | Sets the zero-force aerodynamic neutral position during flight. |

| **Max Centering Force** | Vertical Slider | $0\text{--}25\text{ kg}$ | `config_st->payloadPedalConfig_st.maxForce` | Sets the maximum resistance of the centering spring for both yaw and toe brake stroke. |
...
### 7.4 Shared Bilateral Coupling & Admittance Parameters



Airplane, Toe Brake, and Helicopter modes share the underlying 4 kHz bilateral admittance mechanics and push-pull interconnect:
...
| **Hard Common-Mode Lock** | Firmware Active | $1200\text{ N}$ barrier | `K_COMMON_LOCK_N = 1200.0f` | Solid mechanical barrier that instantly stops dual forward travel if the pilot presses both pedals simultaneously ($(x_L + x_R) > 1.0$). *(Decoupled when Toe Brake is Active)*. |
...
### 7.5 ESP-NOW Wireless Telemetry & Diagnostics
