...
namespace User.PluginSdkDemo
{
    public partial class DIY_FFB_Pedal : IPlugin, IDataPlugin, IWPFSettingsV2
    {
        public class ProfileService
        {
            ...
            public void ApplyProfileAuto(string carName)
            {
            ...
            public void ApplyProfile(string profilePath)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    if (tmpProfile.ConfigPath[i] != "" && File.Exists(tmpProfile.ConfigPath[i]))
                    {
                        ...
                        {
                            _plugin.Settings.ABS_enable_flag[i] = 1;
                        }
                        else
                        {
                            _plugin.Settings.ABS_enable_flag[i] = 0;
                        }

                        if (tmpProfile.Effects[i][1])
                        {
                            _plugin.Settings.RPM_enable_flag[i] = 1;
                        }
                        else
                        {
                            _plugin.Settings.RPM_enable_flag[i] = 0;
                        }

                        if (tmpProfile.Effects[i][3])
                        {
                            _plugin.Settings.G_force_enable_flag[i] = 1;
                        }
                        else
                        {
                            _plugin.Settings.G_force_enable_flag[i] = 0;
                        }

                        if (tmpProfile.Effects[i][4])
                        {
                            _plugin.Settings.WS_enable_flag[i] = 1;
                        }
                        else
                        {
                            _plugin.Settings.WS_enable_flag[i] = 0;
                        }

                        if (tmpProfile.Effects[i][5])
                        {
                            _plugin.Settings.Road_impact_enable_flag[i] = 1;
                        }
                        else
                        {
                            _plugin.Settings.Road_impact_enable_flag[i] = 0;
                        }
